...
void GameApp::UpdateScene(float dt)
{
	...
	XMStoreFloat3(&adjustedPos, XMVectorClamp(cam1st->GetPositionXM(), XMVectorSet(-9.9f, 0.0f, -9.9f, 0.0f), XMVectorReplicate(8.9f)));
	...
	if (ray.Hit(m_Cube.GetBoundingOrientedBox()))
	{
		hitObject = true;
	}
	...
	if (m_MouseTracker.rightButton == Mouse::ButtonStateTracker::PRESSED)
	{
		...
		model.modelParts[0].material.ambient = XMFLOAT4(0.4f, 0.2f, 0.1f, 1.0f);
		model.modelParts[0].material.diffuse = XMFLOAT4(0.2f, 0.5f, 0.1f, 1.0f);
		model.modelParts[0].material.specular = XMFLOAT4(0.2f, 0.2f, 0.6f, 1.0f);
		model.modelParts[0].material.reflect = XMFLOAT4(0.2f, 0.2f, 0.1f, 10.0f);
		HR(CreateWICTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\6.jpg", nullptr, model.modelParts[0].texDiffuse.GetAddressOf()));
		m_Cube.SetModel(std::move(model));
		m_Cube.GetTransform().SetPosition(m_pCamera->GetPosition().x, -2.0f, m_pCamera->GetPosition().z);
	}
	if (m_MouseTracker.leftButton == Mouse::ButtonStateTracker::PRESSED)
	{
		Model model;
		model.SetMesh(m_pd3dDevice.Get(), Geometry::CreateBox(0, 0, 0));
		model.modelParts[0].material.ambient = XMFLOAT4(0.2f, 0.2f, 0.1f, 1.0f);
		model.modelParts[0].material.diffuse = XMFLOAT4(0.2f, 0.2f, 0.1f, 1.0f);
		model.modelParts[0].material.specular = XMFLOAT4(0.2f, 0.2f, 0.1f, 1.0f);
		model.modelParts[0].material.reflect = XMFLOAT4(0.2f, 0.2f, 0.1f, 10.0f);
		HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\bricks.dds", nullptr, model.modelParts[0].texDiffuse.GetAddressOf()));
		m_Cube.SetModel(std::move(model));
		m_Cube.GetTransform().SetPosition(0, 0, 0);
	}
...
bool GameApp::InitResource()
{
	...
	//camera->LookTo(XMFLOAT3(0.0f, 0.0f, -3.0f), XMFLOAT3(0.0f, 4.0f, 1.0f), XMFLOAT3(0.0f, 1.0f, 0.0f));
	camera->LookAt(XMFLOAT3(0.0f, 0.0f, -3.0f), XMFLOAT3(0.0f, 0.0f, 1.0f), XMFLOAT3(0.0f, 1.0f, 0.0f));
